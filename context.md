...
- Active feature branch: `feat/markdown-image-localizer`, cut from `e8bdec6`. Head is now `c696737` after the doc-only v2 revision commit; v3 revision commit to follow. [git:branch]
...
- Two-form output: `LocalizeResult` returns both `rewrittenSourceMarkdown` (`../../wiki/media/...` paths, for `raw/sources/`) and `rewrittenWikiMarkdown` (`../media/...` paths, for `wiki/sources/`). Neither can be computed from the other by string replace. [user:decision-2026-07-23-v3]
